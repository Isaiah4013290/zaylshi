...
export function AdminActions({ leagueId, league, questions, pendingMembers, approvedMembers, isSuperAdmin }: Props) {
  ...
  const createQuestion = async (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      ...
      if (res.ok) {
        ...
        setQuestionText(''); setCloseDate(''); setCloseTime('')
        router.refresh()
      } else flash('❌ Error')
    })
  }
  ...
  const deleteQuestion = async (questionId: string) => {
    if (!isSuperAdmin) { flash('❌ Only super admin can delete questions'); return }
          ...
          <form onSubmit={createQuestion} className="card p-6 space-y-4">
            <div>
              ...
              <textarea value={questionText} onChange={e => setQuestionText(e.target.value)}
                ...
                placeholder="Will Freeman win on Friday?" required />
            </div>
                ...
                <div key={m.id} className="flex items-center justify-between px-4 py-3 border-b border-[#1f1f1f]/40">
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`text-xs px-2 py-0.5 rounded-full ${
                      ...
                      m.role === 'admin' ? 'bg-blue-900/40 text-blue-400' : 'bg-[#1f1f1f] text-gray-500'
                    }`}>{m.role}</span>
                  </div>
